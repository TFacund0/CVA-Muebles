...
final class VentasControllerFeatureTest extends CIUnitTestCase
{
    ...
    // -------------------- VTA-04 (documentación de comportamiento actual) --------------------

    public function testCarritoComprarConfiaEnElPrecioDelCarritoNoRevalidaPrecioActual(): void
    {
        $sesionConCarrito = $this->sembrarCarrito();
        ...
        $productoModel->update($this->productoActivo['id_producto'], ['precio_vta' => $precioOriginal + 500]);
        ...
        try {
            ...
            // DOCUMENTA el comportamiento actual: se confía en el precio del
            // carrito, NO se relee precio_vta al momento de comprar. Esto NO
            // es una aserción de que el comportamiento sea correcto (ver
            // spec: no-goal #1), solo documenta el estado presente.
            $this->assertEquals($precioCarrito, (float) $venta['total_venta']);
            $this->assertNotEquals((float) $precioOriginal + 500, (float) $venta['total_venta']);
        } finally {

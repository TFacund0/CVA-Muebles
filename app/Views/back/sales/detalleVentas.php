...
<div class="row g-3 g-md-4 mb-5">
    <div class="col-6 col-md-3">
        <div class="admin-card-v2 p-3 p-md-4 border-start border-4 border-info h-100 shadow-sm cursor-pointer js-filter-status" data-status="ALL">
    ...
    <div class="col-6 col-md-3">
        <div class="admin-card-v2 p-3 p-md-4 border-start border-4 h-100 shadow-sm cursor-pointer admin-kpi-accent-blue js-filter-status" data-status="EN_PROCESO">
            <div class="d-flex justify-content-between align-items-center">
                <div>
    ...
    <div class="col-6 col-md-3">
        <div class="admin-card-v2 p-3 p-md-4 border-start border-4 border-warning h-100 shadow-sm cursor-pointer js-filter-status" data-status="PENDIENTE">
            <div class="d-flex justify-content-between align-items-center">
                <div>
        ...
        <div class="admin-card-v2 p-3 p-md-4 border-start border-4 border-success h-100 shadow-sm">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <h4 class="fw-bold text-success mb-0">$ <?= money($counts['ingresos'], 0) ?></h4>
                    <span class="d-block text-muted text-2xs">Total de pagos cobrados</span>
                </div>
                                ...
                                <td class="ps-4" data-label="CLIENTE">
                                    <div class="d-flex align-items-center gap-3 py-1 order-info-wrapper">
                                        <div class="position-relative">
                                            ...
                                            <span class="position-absolute top-0 start-0 badge rounded-pill bg-dark shadow-sm d-lg-none badge-corner-id">#<?= $v['id'] ?></span>
                                        </div>
                                        <div class="order-text-details">
                                            <div class="fw-bold text-cva-brown"><?= esc(($v['nombre'] ?? 'VENTA') . ' ' . ($v['apellido'] ?? 'MANUAL')) ?></div>
                                            <div class="d-flex gap-2 align-items-center">
                                                <span class="badge bg-light text-muted border d-none d-md-inline-block text-2xs">PEDIDO: #<?= $v['id'] ?></span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="d-none d-lg-table-cell" data-label="FECHA">
                                    <div class="fw-bold text-cva-brown"><?= fecha_local($v['fecha'], 'd/m/Y') ?></div>
                                    <div class="x-small text-muted"><?= fecha_local($v['fecha'], 'H:i') ?> hs</div>
                                </td>
                                <td class="text-end" data-label="TOTAL / PAGADO">
                                    <div class="order-total-info">
                                        <div class="fw-bold text-dark mb-1">
                                            $ <?= money($v['total_venta'], 0) ?>
                                        </div>
                                        <div class="x-small">
                                            <?php if ($v['total_pagado'] >= $v['total_venta']): ?>
                                                <span class="badge bg-success-soft text-success border border-success-subtle px-2 py-0.5 rounded-pill fw-bold text-2xs">
                                                    <i class="bi bi-check-circle-fill me-1"></i> PAGADO
                                                </span>
                                            <?php elseif ($v['total_pagado'] > 0): ?>
                                                <span class="badge bg-warning-soft text-warning border border-warning-subtle px-2 py-0.5 rounded-pill fw-bold text-2xs">
                                                    COBRADO: $<?= money($v['total_pagado'], 0) ?>
                                                </span>
                                            <?php else: ?>
                                                <span class="badge bg-light text-muted border px-2 py-0.5 rounded-pill fw-bold text-2xs">
                                                    SIN PAGO
                                                </span>
                                            <?php endif; ?>
                                        </div>
                                ...
                                <td class="text-center" data-label="PRIORIDAD">
                                    <div class="d-flex justify-content-center align-items-center gap-1">
                                        <a href="<?= base_url('ventas/subir/' . $v['id']) ?>"
                                           class="btn btn-action-premium text-gold border-gold border-opacity-25 shadow-sm p-0 d-flex align-items-center justify-content-center btn-prioridad-subir priority-btn-32"
                                           title="Subir prioridad">
                                            <i class="bi bi-arrow-up fs-6"></i>
                                        </a>
                                        <a href="<?= base_url('ventas/bajar/' . $v['id']) ?>"
                                           class="btn btn-action-premium text-gold border-gold border-opacity-25 shadow-sm p-0 d-flex align-items-center justify-content-center btn-prioridad-bajar priority-btn-32"
                                ...
                                <tr class="solicitud-row">
                                    <td class="ps-4 col-solicitud-cliente" data-label="CLIENTE">
                                        <div class="d-flex align-items-center gap-3 py-1 order-info-wrapper">
                                            <div class="position-relative">
                                                ...
                                                <span class="position-absolute top-0 start-0 badge rounded-pill bg-dark shadow-sm d-lg-none badge-corner-id">#<?= $s['id'] ?></span>
                                            </div>
                                            <div class="order-text-details">
                                                <div class="fw-bold text-cva-brown"><?= esc($s['nombre'] . ' ' . $s['apellido']) ?></div>
                                                <div class="d-flex gap-2 align-items-center mt-1">
                                                    <span class="badge bg-light text-muted border d-none d-md-inline-block text-2xs">ID: #<?= $s['id'] ?></span>
                                                    <span class="badge bg-gold-soft text-gold border border-gold border-opacity-25 x-small text-3xs"><?= esc($s['email']) ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="small fw-bold col-solicitud-fecha d-none d-lg-table-cell" data-label="FECHA"><?= fecha_local($s['fecha'], 'd/m/Y H:i') ?></td>
                                    <td class="text-end fw-bold text-dark col-solicitud-monto" data-label="MONTO">$ <?= money($s['total_venta'], 2) ?></td>
                                    <td class="text-center pe-4 col-solicitud-acciones" data-label="GESTIÓN">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <form action="<?= base_url('ventas/actualizar_estado/' . $s['id']) ?>" method="post" class="d-inline" data-confirm="¿Estás seguro de rechazar este pedido? No se mostrará en la lista.">
                                                <?= csrf_field() ?>
...
<script src="<?= base_url('assets/js/admin/admin-detalle-ventas.js?v=1.0') ?>"></script>
<?= $this->endSection() ?>

...
<?= $this->section('extra-css') ?>
    <link rel="stylesheet" href="<?= base_url('assets/css/admin/admin-products.css?v=4.0')?>">
<?= $this->endSection() ?>
...
<div class="admin-card-v2 border-0 shadow-sm overflow-hidden mb-5">
    <div class="table-responsive-stack">
        <table class="table table-hover align-middle mb-0" id="products-table" data-initial-view="<?= esc($vista) ?>">
            <thead class="bg-brown text-white">
                <tr>
                    <th class="ps-4 py-4 text-gold x-small fw-bold border-0 text-uppercase tracking-widest col-product-piece">PIEZA Y ESPECIFICACIONES</th>
            ...
            <tbody id="table-body">
                    ...
                    <?php foreach ($productos as $p): ?>
                        ...
                        <tr class="product-row"
                                ...
                                <div class="d-flex align-items-center gap-3 py-3 product-info-wrapper">
                                    ...
                                    <div class="product-text-details">
                                        ...
                                        <div class="d-flex gap-2">
                                            <span class="x-small fw-bold text-uppercase badge bg-light text-muted border px-2 py-1 text-4xs">ID: <?= $p['id_producto'] ?></span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="text-end" data-label="PRECIO">
                                <div class="fw-bold text-cva-brown mb-0">$<?= money($p['precio_vta'], 2) ?></div>
                            </td>
                            <td class="text-center" data-label="GESTIÓN">
                                <div class="d-flex flex-column flex-sm-row flex-lg-column gap-2 align-items-center justify-content-center w-100">
                                    ...
                                    <div class="w-100 btn-group-actions">
                                        <button type="button" class="btn btn-action-premium text-danger border-danger border-opacity-25 w-100 action-archive js-submit-action <?= $p['eliminado'] == 'SI' ? 'd-none' : '' ?>"
                                                data-url="<?= base_url('/delete-producto/' . $p['id_producto']) ?>"
                                                data-confirm-msg="¿Confirmas archivar esta pieza del catálogo?">
                                            <i class="bi bi-archive-fill me-2"></i> ARCHIVAR
                                        </button>
                                        <button type="button" class="btn btn-action-premium text-success border-success border-opacity-25 w-100 action-restore js-submit-action <?= $p['eliminado'] == 'NO' ? 'd-none' : '' ?>"
                                                data-url="<?= base_url('/activar-producto/' . $p['id_producto']) ?>"
                                                data-confirm-msg="¿Confirmas reactivar esta pieza?">
                                            <i class="bi bi-arrow-counterclockwise me-2"></i> RESTAURAR
                                        </button>
                                        <button type="button" class="btn btn-action-premium text-danger border-danger border-opacity-25 w-100 mt-2 action-delete-permanent js-submit-action <?= $p['eliminado'] == 'NO' ? 'd-none' : '' ?>"
                                                data-url="<?= base_url('/eliminar-producto-permanente/' . $p['id_producto']) ?>"
                                                data-confirm-msg="¿Confirmas eliminar PERMANENTEMENTE este mueble del catálogo? Esta acción no se puede deshacer y borrará toda la información asociada.">
                ...
                <tr id="no-results-row" class="admin-hidden-row">
                    <td colspan="4" class="text-center py-5 bg-light bg-opacity-50">
                        <div class="py-5 text-center">
                            ...
                            <button type="button" id="btn-reset-filters-empty" class="btn btn-outline-gold rounded-pill px-4 mt-3 animate-pulse-gold">Limpiar búsqueda</button>
                        </div>
                    </td>
                </tr>
                ...
                <tr id="empty-archive-row" class="admin-hidden-row">
                    <td colspan="4" class="text-center py-5 bg-light bg-opacity-50">
                        <div class="py-5 text-center">
...
<script src="<?= base_url('assets/js/admin/admin-crud-productos.js?v=1.0') ?>"></script>
<?= $this->endSection() ?>

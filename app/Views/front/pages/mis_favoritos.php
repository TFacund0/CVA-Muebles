...
<link rel="stylesheet" href="<?= base_url('assets/css/pages/favoritos.css?v=3.0') ?>">
<?= $this->endSection() ?>

<?= $this->section('content') ?>
<div id="favoritos-page"
    data-csrf-token="<?= csrf_hash() ?>"
    data-favoritos-toggle-url="<?= base_url('favoritos/toggle/') ?>">
<header class="fav-header text-center">
    <div class="container animate-fade-in">
        <span class="text-gold fw-bold text-uppercase x-small fav-kicker">Tu Selección Personal</span>
        <h1 class="display-3 fw-bold font-lora mt-2">Mis Favoritos</h1>
        <div class="mx-auto mt-3 fav-header-divider"></div>
    </div>
</header>
...
<div class="container mb-5 pb-5">
    <?php if (empty($favoritos)): ?>
        <div class="empty-state-fav text-center">
            <div class="mb-4">
                <i class="bi bi-heart-break text-muted opacity-25 fav-empty-icon"></i>
            </div>
    ...
    <?php else: ?>
        ...
        <div class="row mb-5 g-3 align-items-center animate-fade-in">
            ...
            <div class="col-lg-5 col-md-6">
                <div class="input-group bg-white rounded-pill px-3 py-2 border shadow-sm transition-all focus-within-gold fav-search-box">
                    <span class="input-group-text bg-transparent border-0 py-0"><i class="bi bi-search text-muted"></i></span>
                    <input type="text" id="search-favs" class="form-control border-0 bg-transparent py-0 fav-search-input" placeholder="Buscar entre tus favoritos..." aria-label="Buscar favoritos">
        ...
        <div id="no-results-fav" class="empty-search-state d-none mb-5 animate-fade-in">
            <div class="mb-4">
                <i class="bi bi-search text-gold opacity-25 fav-noresults-icon"></i>
            </div>
                ...
                <div class="col-lg-4 col-md-6 fav-item" data-categorias="<?= esc($cat_lower) ?>" data-nombre="<?= esc($nombre_lower) ?>">
                    <div class="fav-card">
                        <div class="fav-img-wrapper">
                            <img src="<?= imagen_url($fav['imagen']) ?>" alt="<?= $fav['nombre_prod'] ?>">
                            <button class="remove-fav-btn shadow-sm js-remove-fav" data-producto-id="<?= $fav['producto_id'] ?>" title="Quitar de favoritos">
                                <i class="bi bi-trash3-fill"></i>
                            </button>
                        </div>
                        <div class="p-4 d-flex flex-column fav-card-body">
                            <div class="mb-2 d-flex justify-content-between align-items-center">
                                <span class="x-small fw-bold text-gold text-uppercase fav-card-kicker">Pieza de Autor</span>
                                <?php if (!empty($fav['categoria'])): ?>
                                    <span class="badge bg-light text-muted x-small shadow-sm fav-cat-badge"><?= esc($fav['categoria']) ?></span>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="mt-auto">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <span class="price-tag-fav">$<?= money($fav['precio_vta'], 0) ?></span>
...
<script src="<?= base_url('assets/js/pages/mis-favoritos.js?v=1.0') ?>"></script>
<?= $this->endSection() ?>